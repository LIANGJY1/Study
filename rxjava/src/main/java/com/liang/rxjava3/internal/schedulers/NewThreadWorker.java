...
public class NewThreadWorker extends Scheduler.Worker {
    ...
    public ScheduledRunnable scheduleActual(final Runnable run, long delayTime, @NonNull TimeUnit unit, @Nullable DisposableContainer parent) {
        Runnable decoratedRun = RxJavaPlugins.onSchedule(run);// run: "1"

        ScheduledRunnable sr = new ScheduledRunnable(decoratedRun, parent);// parent:CompositeDisposable decoratedRun:"1"

        if (parent != null) {
            if (!parent.add(sr)) {// false
                return sr;
            }
        }

        Future<?> f;// f: java.util.concurrent.ScheduledThreadPoolExecutor$ScheduledFutureTask@684311a[Completed normally]
        try {
            if (delayTime <= 0) {// delayTime 0
                f = executor.submit((Callable<Object>)sr);// java.util.concurrent.ScheduledThreadPoolExecutor@1cb5a3c[Running, pool size = 0, active threads = 0, queued tasks = 0, completed tasks = 0]
            } else {
        ...
        return sr;// ScheduledRunnable，没用
    }
