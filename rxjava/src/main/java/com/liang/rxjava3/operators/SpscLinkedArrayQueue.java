...
import com.liang.rxjava3.internal.util.Pow2;
...
public final class SpscLinkedArrayQueue<T> implements SimplePlainQueue<T> {
     ...
     */
    @Override
    public boolean offer(final T e) {// e 1
        if (null == e) {
            throw new NullPointerException("Null is not a valid element");
        }
        // local load of field to avoid repeated loads after volatile reads
        final AtomicReferenceArray<Object> buffer = producerBuffer; // producerBuffer null
        final long index = lpProducerIndex();// 0
        final int mask = producerMask;// producerMask 127
        final int offset = calcWrappedOffset(index, mask);// 0
        if (index < producerLookAhead) {// producerLookAhead 126
            return writeToQueue(buffer, e, index, offset);// buffer；null e 1
        } else {
