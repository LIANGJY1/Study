...
Instructs a reactive type to invoke the given `com.liang.functions.Consumer` when it encounters an error.
...
Optionally, a `com.liang.functions.Predicate` can be specified that gives more control over when an error event should be replaced by a completion event, and when not.
...
Instructs a reactive type to emit the item returned by the specified `com.liang.functions.Function` when it encounters an error.
...
Instructs a reactive type to resubscribe to the source reactive type if it encounters an error until the given `com.liang.functions.BooleanSupplier` returns `true`.
