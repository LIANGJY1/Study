...
To allow having RxJava 1.x and RxJava 2.x side-by-side, RxJava 2.x is under the maven coordinates `com.liang.rxjava2:rxjava:2.x.y` and classes are accessible below `io.reactivex`.
...
We try to remedy this situation in 2.x by having `com.liang.Observable` non-backpressured and the new `com.liang.Flowable` be the backpressure-enabled base reactive class.

The good news is that operator names remain (mostly) the same. The bad news is that one should be careful when performing 'organize imports' as it may select the non-backpressured `com.liang.Observable` unintended.
...
Similar to the naming changes, `rx.Completable.CompletableSubscriber` has become `com.liang.CompletableObserver` with `onSubscribe(Disposable)`:
...
The `com.liang.subjects.AsyncSubject`, `com.liang.subjects.BehaviorSubject`, `com.liang.subjects.PublishSubject`, `com.liang.subjects.ReplaySubject` and `com.liang.subjects.UnicastSubject` in 2.x don't support backpressure (as part of the 2.x `Observable` family).

The `com.liang.processors.AsyncProcessor`, `com.liang.processors.BehaviorProcessor`, `com.liang.processors.PublishProcessor`, `com.liang.processors.ReplayProcessor` and `com.liang.processors.UnicastProcessor` are backpressure-aware. The `BehaviorProcessor` and `PublishProcessor` don't coordinate requests (use `Flowable.publish()` for that) of their downstream subscribers and will signal them `MissingBackpressureException` if the downstream can't keep up. The other `XProcessor` types honor backpressure of their downstream subscribers but otherwise, when subscribed to a source (optional), they consume it in an unbounded manner (requesting `Long.MAX_VALUE`). 
...
The `rx.observables.ConnectableObservable` is now `com.liang.observables.ConnectableObservable<T>` and `com.liang.flowables.ConnectableFlowable<T>`.

## GroupedObservable

The `rx.observables.GroupedObservable` is now `com.liang.observables.GroupedObservable<T>` and `com.liang.flowables.GroupedFlowable<T>`.
...
Because both 1.x and 2.x is aimed at Java 6+, we can't use the Java 8 functional interfaces such as `function.Function`. Instead, we defined our own functional interfaces in 1.x and 2.x follows this tradition. 
...
The remaining action interfaces were named according to the Java 8 functional types. The no argument `Action0` is replaced by the `com.liang.functions.Action` for the operators and `java.lang.Runnable` for the `Scheduler` methods. `Action1` has been renamed to `Consumer` and `Action2` is called `BiConsumer`. `ActionN` is replaced by the `Consumer<Object[]>` type declaration.

## Functions

We followed the naming convention of Java 8 by defining `com.liang.functions.Function` and `com.liang.functions.BiFunction`, plus renaming `Func3` - `Func9` into `Function3` - `Function9` respectively. The `FuncN` is replaced by the `Function<Object[], R>` type declaration.
...
To avoid the name clash, the 1.x `rx.Subscription` has been renamed into `com.liang.Disposable` (somewhat resembling .NET's own IDisposable).
...
The 2.x API still supports the main default scheduler types: `computation`, `io`, `newThread` and `trampoline`, accessible through `com.liang.schedulers.Schedulers` utility class. 
...
The `com.liang.Scheduler` abstract base class now supports scheduling tasks directly without the need to create and then destroy a `Worker` (which is often forgotten):
...
Testing RxJava 2.x works the same way as it does in 1.x. `Flowable` can be tested with `com.liang.subscribers.TestSubscriber` whereas the non-backpressured `Observable`, `Single`, `Maybe` and `Completable` can be tested with `com.liang.observers.TestObserver`.
